...
class ExP():
    def __init__(self, nsub):
        ...
        # self.root = 'Data/strict_TE/BCIIV2B/'
        # Create directories if they don't exist
        log_dir = f"results/2b/STDCNN"
        os.makedirs(log_dir, exist_ok=True)
        self.mod_dir = f"./results/2b/STDCNN/save_model/"
        os.makedirs(self.mod_dir, exist_ok=True)

        self.log_write = open(f"{log_dir}/subject{self.nSub}.txt", "w")
    ...
    def get_source_data(self):
        ...
        paradigm = MotorImagery(n_classes=2, fmin=0, fmax=40)
        self.total_data, self.total_label, meta = paradigm.get_data(dataset, subjects=[self.nSub])
        session_T_mask = (meta['session'] == '0train') | (meta['session'] == '1train') | (meta['session'] == '2train')
        ...
        session_E_mask = (meta['session'] == '3test') | (meta['session'] == '4test')
    ...
    def train(self):
        ...
        input_data = torch.randn(1, 1, 3, 1000).cuda()
